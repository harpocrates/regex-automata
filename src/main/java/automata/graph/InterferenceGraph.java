...
/**
 * Interference graph for tracking interference and coalescing of variables.
 *
 * TODO: support rendering this with `DotGraph` (using `layout=circo;` and `graph`)
 *
...
public class InterferenceGraph<V> {
  ...
  /**
   * Mapping from merged variables to the variables into which they were merged.
   *
   * This is aware of chained merges (ex. if {@code a} is merged into {@code b}
   * and {@code b} is merged into {@code c} then the value associated with both
   * {@code a} and {@code b} in the canonical mapping will be {@code c}).
   *
   * The map returned is not modifiable and will not reflect future updates.
   *
   * @return mapping from merged variables to their canonical versions
   */
  public Map<V, V> canonicalMapping() {
   ...
   *
   * @return canonical version of a variable
   */
